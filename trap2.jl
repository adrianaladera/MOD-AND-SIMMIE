...
f̂(x, p::P, t) where P = 
    x - p[1] - p[2] / 2 * p[3](x, p[end], t) - p[2] / 2 * p[3](p[1], p[end], t)
function trapezoid(f::F, t₀, tₙ, x₀, p, n) where F
    ...
    for i ∈ 1:n
        ...
        x[i + 1] = newton(f̂, x[i], (x[i], Δ, f, p), t, 1e-4, 1e-4, 1e-4, 50)[1]
    end
    return x, t
end
...
function newton(f::F, x, p, t, errf, errΔₓ, relΔₓ, itermax) where F
    k = 1
    X = [x]
    fₖ = f(X[k], p, t)
    errfₖ = norm(fₖ, Inf)
    ...
    fₚᵤ(x) = f(x, p, t)
    while k <= itermax && (errfₖ > errf || errΔxₖ > errΔₓ || relΔxₖ > relΔₓ)
        Jf = ForwardDiff.jacobian(fₚᵤ, X[k])
        if length(Jf) == 1
            Jf = Jf[1]
        end
        Δₓ = Jf \ (-fₖ)
